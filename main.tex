...
%%
Pyrethroid insecticides target the voltage-gated sodium channel (VGSC) in the insect nervous system, however changes in the amino acid sequence of the \emph{Vgsc} gene can disrupt the activity of these chemicals, inducing insecticide resistance.
%
Resistance to pyrethroid insecticides is a major concern for malaria control, because these compounds have been successfully used to reduce malaria mortality in insecticide-treated bed-net campaigns.
%
There are fears that insecticide resistance in mosquitoes may reduce the effectiveness of malaria control and also wider concerns that pyrethroid resistance in other insects pose food security threats.
%
To better understand the evolution of pyrethroid resistance we use whole-genome sequence data from the \emph{Anopheles gambiae} 1000 Genomes Project to provide a comprehensive account of genetic variation in the \emph{Vgsc} gene in malaria mosquito populations from eight African countries.
%
We describe 20 amino acid altering genetic variants, previously unknown in these mosquitoes, then analyse the genome around these variants to detect signals of selection and to refine our understanding of resistance spreading between species and geographical locations.
%
Our results demonstrate that the molecular basis of this resistance is more complex than previously appreciated, and provide a foundation for the development of new genetic tools to track the spread insecticide resistance.
%%
...
%
However, we found no systematic difference in the length of shared haplotypes when comparing F1-5 (carrying \texttt{L995F}) against S1-5 (carrying \texttt{L995S}) (\ref{fig:pspd} Figure).
%
There were, however, some differences between core haplotypes carrying the same allele.
%
For example, shared haplotypes were significantly longer for S1 (median 1.091 cM, 95\% CI [1.076 - 1.091]) versus other core haplotypes carrying \texttt{L995S} (e.g., S2 median 0.699 cM, 95\% CI [0.696 - 0.705]; \ref{fig:pspd} Figure).
%
...
%
In S1 Table we list all 64 non-synonymous variants found within the \textit{Vgsc} gene in this study, with population allele frequencies.
%
In S2 Table we list 771 biallelic SNPs, within the \textit{Vgsc} gene and up to 10 kbp upstream or downstream, that are potentially informative regarding which haplotype group a resistance haplotype belongs to, and thus could be used for tracking the spread of resistance.
%
...
%
We also provide S3 Table which lists all 8,297 SNPs found within the \textit{Vgsc} gene and up to 10 kbp upstream or downstream, which might need to be taken into account as flanking variation when searching for PCR primers to amplify a SNP of interest.
%
...
%
S1 Table includes the predicted effect for all SNPs that are non-synonymous in one or more of these transcripts.
%
...
%
For ease of comparison with previous work on \emph{Vgsc}, pan Insecta, in Table \ref{table:variants_missense} and S1 Table we report codon numbering for both \emph{An. gambiae} and \emph{Musca domestica} (the species in which the gene was first discovered).
%
...
%
Analyses of haplotype homozygosity in moving windows (\ref{fig:mhh_f} Figure, \ref{fig:mhh_s} Figure) and pairwise haplotype sharing (\ref{fig:pspd} Figure) were performed using custom Python code available from \url{https://github.com/malariagen/agam-vgsc-report}.
%
...
\begin{figure}[!b]
  ...
  \caption{\textbf{Windowed analysis of haplotype homozygosity for genetic backgrounds carrying the \texttt{L995S} allele}. See \ref{fig:mhh_f} Figure for explanation. Haplotype homozygosity is high between groups S4 and S5 on both flanks of the gene, indicating that haplotypes from both groups are in fact closely related.}
  \label{fig:mhh_s}
\end{figure}
%%
